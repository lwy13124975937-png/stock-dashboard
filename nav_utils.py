...
def stock_daily_latest(code):
    ...
    def fetch():
        ...
        return close, str(row["date"].date()), (close / prev - 1) * 100 if prev else None
...
def akshare_open_latest(code):
    ...
    if growth_col and pd.notna(row.get(growth_col)):
        ...
        prev = float(df.iloc[-2][nav_col]) if len(df) > 1 else nav
        change_pct = (nav / prev - 1) * 100 if prev else None
    return nav, str(row[date_col].date()), change_pct
...
def get_stock_nav(code):
    try:
        nav, date, change_pct = stock_daily_latest(code)
        result = NavResult(clean_code(code), "A股股票", "新浪A股日线", nav, date, "真", change_pct)
...
def get_exchange_fund_nav(code):
    ...
    errors = []
    try:
        nav, date, change_pct = sina_quote(sina_fund_symbol(code))
        result = NavResult(code, "场内基金", "新浪实时行情", nav, date, "真", change_pct)
        write_nav_cache(result)
        return result
    except Exception as e:
        errors.append(f"新浪实时失败：{str(e)[:80]}")
    try:
        nav, date, change_pct = exchange_fund_daily_latest(code)
        result = NavResult(code, "场内基金", "新浪场内基金日线", nav, date, "真", change_pct, "实时失败后回退日线")
        write_nav_cache(result)
        return result
    except Exception as e:
...
def get_otc_a_share_nav(code, name=""):
    ...
    try:
        ...
        if after_close and true_nav and true_date == today:
            result = NavResult(code, "场外基金-A股主题", "天天基金fundgz-dwjz", true_nav, true_date, "真", est_pct)
            write_nav_cache(result)
            return result
        if est_nav and est_date == today:
            result = NavResult(code, "场外基金-A股主题", "天天基金fundgz-gsz", est_nav, est_date, "估", est_pct)
            write_nav_cache(result)
            return result
        if true_nav and true_date:
            result = NavResult(code, "场外基金-A股主题", "天天基金fundgz-dwjz", true_nav, true_date, "真", None, "最新已披露真净值，日期按接口返回")
            write_nav_cache(result)
            return result
    ...
    try:
        nav, date, change_pct = akshare_open_latest(code)
        result = NavResult(code, "场外基金-A股主题", "AkShare单位净值走势", nav, date, "真", change_pct, "fundgz失败后回退AkShare")
        write_nav_cache(result)
        return result
    except Exception as e:
...
def get_otc_qdii_nav(code, classify="场外基金-QDII/境外"):
    ...
    try:
        nav, date, change_pct = akshare_open_latest(code)
        result = NavResult(code, classify, "AkShare单位净值走势", nav, date, "真", change_pct, "QDII/境外净值按基金披露节奏滞后更新")
        write_nav_cache(result)
        return result
    except Exception as e:
        errors.append(f"AkShare净值失败：{str(e)[:100]}")
    try:
        ...
        true_date = str(data.get("jzrq") or "").strip()
        if true_nav and true_date:
            result = NavResult(code, classify, "天天基金fundgz-dwjz", true_nav, true_date, "真", None, "AkShare失败后回退fundgz；QDII净值披露可能滞后")
            write_nav_cache(result)
            return result
...
def current_holdings_self_check():
    ...
    for h in HOLDINGS:
        ...
        nav_text = "—" if result.nav is None else f"{result.nav:.4f}"
        rows.append({
...
if __name__ == "__main__":
    df = current_holdings_self_check()
